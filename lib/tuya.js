...
async function sendDataPoints(entity, dpValues, cmd, seq=undefined) {
    ...
    var disableDefaultResponse = true;
    if (entity.hasOwnProperty('meta')
        && entity.meta.hasOwnProperty('disableDefaultResponse')
        && entity.meta.disableDefaultResponse === false) {
...
function async configureDeviceForRequestQueuing(device, coordinatorEndpoint, logger){
    const endpoint = device.getEndpoint(1);
    var meta = endpoint.hasOwnProperty('meta') ? endpoint.meta : {};
    if (!meta.hasOwnProperty('disableDefaultResponse')) {
...
module.exports = {
    ...
    configureDeviceForRequestQueuing,
};
